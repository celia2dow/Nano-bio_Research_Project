...
while t < total_t && all(cell_phases == K*ones(1, numel(culture_dish)), 'all') == 0 
    ...
    probs_per_cell = cycle_probs(cell_phases(1:N_t));
    chosen_to_transition = cell_sites(Bernoulli_nums <= probs_per_cell);
    for j = 1:length(chosen_to_transition(chosen_to_transition>0))
        parent_site = chosen_to_transition(j);
        ...
        if old_phase == K   % A cell at the end of it's cycle (ready to
            ...
            if culture_dish(daughter_site) == 0
                ...
                % If a cell in phase K doesn't successfully proliferate, it
                % remains in state K
                new_phase = K; 
            end
        else
        ...
        cell_phases(cell_sites == parent_site) = new_phase;
