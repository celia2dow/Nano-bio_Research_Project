...
if total.cell_population(1)==total.cell_population(end)
    ...
    % Need to divide the Poisson MLE by the cell population and multiply 
    % lambda_1 by the confluence
    prtcls_obs.MLE_Poisson = est_lambda1.MLE .* binrng(2) ...
        .* PARAMETERS.initial_num_cells .* freePrtcls_start_of_t ./ ...
        (PARAMETERS.prtcls_per_site .* PARAMETERS.culture_dim.^2);
    est_lambda1.MLE_Poisson = log(1-prtcls_obs.MLE_Poisson)./...
        (-binrng(2).*total.confluence(1));
    ...
    % Upper bound
    prtcls_obsUP.MLE_Poisson = est_lambda1UP.MLE .* binrng(2) ...
        .* PARAMETERS.initial_num_cells .* freePrtcls_start_of_tLO ./ ...
        (PARAMETERS.prtcls_per_site .* PARAMETERS.culture_dim.^2);
    ...
    % Lower bound
    prtcls_obsLO.MLE_Poisson = est_lambda1LO.MLE .* binrng(2) ...
        .* PARAMETERS.initial_num_cells .* freePrtcls_start_of_tUP ./ ...
        (PARAMETERS.prtcls_per_site .* PARAMETERS.culture_dim.^2);
